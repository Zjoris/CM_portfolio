...
tracks <- tracks %>%
  mutate(
    ...
    Weeknmbr = ifelse(type=="Daily Mix",get_week_n(Day),substr(playlist_name,7,7))
  ) 
```
...
tracks %>%
  ...
  ggplot(aes(x = valence, y = energy,color = playlist_name,shape=mode)) + geom_point(alpha=0.5)
```
