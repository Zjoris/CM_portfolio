...
In terms of energy/valence the discover weeklies are distributed fairly similar.
Some deviations are for instance week 4 and week 6, that seems to have its energy more concentrated in the center. Week 6 also seems to be more optimistic in general. 
All playlists consist of mostly major songs which, honestly, was to my surprise. 
Something interesting in weekly 2 is that the songs with highest valence and highest energy are songs in a minor key.
Also week 7 shows something remarkable. The happiest most energetic songs here seem to be in the minor keys whilst the songs with lower energy and lower valence seem to be in a major key. 
...
```{r}
tracks0702_forest %>% get_pr() %>% kable()
...
<!-- Possibly I should combine key and mode to get a feature that consists of all 24 possibilities of key_modes.  -->
...
tracksMFCC <- tracks %>% 
  ...
  bind_rows(tracks %>% filter(playlist_name == "Weekly7")) %>% 
  count(track.uri,key) %>%
...
W7[-nrow(W7),] %>% 
  ...
  mutate(weight = w)%>% 
  datatable(extensions = "Buttons", options = list(dom="Blfrtip",buttons = c('copy','csv','print'),lengthMenu = list(c(6,12,24,48,-1),c(6,12,24,48,"All"))))
```
...
W7b %>% 
  ...
  rename(DW = V1) %>%
  datatable(extensions = "Buttons", options = list(dom="Blfrtip",buttons = c('copy','csv','print'),lengthMenu = list(c(12,24,48,-1),c(12,24,48,"All"))))
